...
const Layout: React.FC<LayoutProps> = ({ children, currentView, onNavigate }) => {
  ...
  return (
    <div className="min-h-screen bg-dl-bg dark:bg-dl-dark flex flex-col max-w-md mx-auto transition-colors duration-300 relative">
      <header className="sticky top-0 z-[100] bg-white/80 dark:bg-dl-dark-s/80 backdrop-blur-lg shadow-soft dark:shadow-none border-b border-gray-100/80 dark:border-dl-dark-b/80 px-5 py-3.5 flex items-center justify-between transition-colors duration-300">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[10px] text-gray-400 dark:text-dl-dt2 leading-none mt-0.5">AI-assisted diagnostics</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full border ${
            online
              ? "bg-green-50 dark:bg-success-d/10 border-green-100 dark:border-success-d/20"
              : "bg-gray-100 dark:bg-dl-dark-s2 border-gray-200 dark:border-dl-dark-b"
          }`}>
            <span className={`w-1.5 h-1.5 rounded-full ${online ? "bg-success dark:bg-success-d" : "bg-gray-400"}`} />
            <span className={`text-[10px] font-semibold ${online ? "text-success dark:text-success-d" : "text-gray-500 dark:text-dl-dt2"}`}>
      ...
      <nav className="fixed bottom-0 left-0 right-0 max-w-md mx-auto z-[200] bg-white dark:bg-dl-dark-s border-t border-gray-100 dark:border-dl-dark-b shadow-elevated dark:shadow-none h-20 flex items-center justify-around px-2 transition-colors duration-300 pb-safe">
...
const NavItem: React.FC<NavItemProps> = ({ view, icon, label, current, onNavigate }) => {
  ...
  return (
    ...
    >
      ...
      <span className={`material-symbols-outlined text-2xl relative z-10 transition-colors ${
        active ? "text-primary dark:text-accent" : "text-gray-400 dark:text-dl-dt2 hover:text-gray-600 dark:hover:text-dl-dt"
      }`}>
        {icon}
      </span>

      <span className={`text-[10px] font-semibold relative z-10 transition-colors ${
        active ? "text-primary dark:text-accent" : "text-gray-400 dark:text-dl-dt2"
      }`}>
        {label}
      </span>
